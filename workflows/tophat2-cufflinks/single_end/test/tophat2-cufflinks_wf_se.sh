#!/bin/bash
# tophat2-cufflinks_wf_se.sh --id <path to id list> --index <path to one of bowtie2 index files (e.g. genome.fa)> --annotation <path to annotation gtf file> [--cwl <path to tophat2-cufflinks_wf_se.cwl>] --yml [<path to tophat2-cufflinks_wf_se.yaml.sample>]
#
set -e
...
BOWTIE2_INDEX_FILE_PATH="${BASE_DIR}/bowtie2_GRCh38/genome.fa"
...
while test $# -gt 0; do
  ...
  case ${key} in
    ...
    --index) BOWTIE2_INDEX_FILE_PATH="$(get_abs_path ${2})"; shift ;;
