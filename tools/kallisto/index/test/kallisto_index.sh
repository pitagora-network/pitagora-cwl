#!/bin/bash
# kallisto_index.sh <path to fasta file> <index_name> <path to kallisto_index.cwl> <path to kallisto_index.yml.sample>
#
set -e
...
yaml_path="${result_dir}/kallisto_index.yml"
cp "${YAML_TMP_PATH}" "${yaml_path}"

sed -i.buk \
  -e "s:_PATH_TO_FASTA_:${FASTA_FILE_PATH}:" \
  -e "s:_INDEX_NAME_:${INDEX_NAME}:" \
  "${yaml_path}"
